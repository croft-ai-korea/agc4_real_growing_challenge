...
class LetsgrowDao:
    ...
    def insert_day_bakup(self, data):
        if ('Message' in data):
            ...
            for key in data:
                ...
                sql = '''
                INSERT INTO simulation ("time", ''' + data + ''')
    ...
    def from_db_day(self, begin_time: datetime):
        sql = " SELECT * from simulation where time >= %(f)s and time < %(t)s "
    ...
    def from_db(self, begin_time: datetime, end_time: datetime):
        sql = " SELECT * from simulation where time >= '" + begin_time.strftime(
    ...
    def make_insert_sql_by_dict(self, anItem):
        ...
        return f"""
            INSERT INTO simulation ( {colNames} )
            VALUES ( {values})
